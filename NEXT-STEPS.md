...
- **Current version:** v1.8.1 (in `manifest.json` and the side-panel header)
...
autofill.js     Standalone engine injected into NON-LinkedIn ATS pages (and cross-origin iframes).
                ...
                stopPatterns. Workday adapter = prompt-option dropdowns (EEO + non-EEO questions via
                the AI answer flow) + account-gate auto-create + email-verify wall; Greenhouse/Lever
                adapter = location typeahead pick.
