...
- **Current version:** v1.8.4 (in `manifest.json` and the side-panel header)
...
autofill.js     Standalone engine injected into NON-LinkedIn ATS pages (and cross-origin iframes).
                ...
                typeahead pick; iCIMS adapter = detection + account-gate auto-create + email-verify
                wall (mostly standard HTML the generic engine already fills). Shared account-gate
                helpers: tickAccountAgreement(), isVerifyEmailWall() (Workday + iCIMS).
