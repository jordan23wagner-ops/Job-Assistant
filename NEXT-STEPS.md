...
- **Current version:** v1.8.5 (in `manifest.json` and the side-panel header)
...
autofill.js     Standalone engine injected into NON-LinkedIn ATS pages (and cross-origin iframes).
                ...
                account-gate auto-create + email-verify wall; Greenhouse/Lever/Ashby/SmartRecruiters
                adapter = location typeahead pick; iCIMS/Taleo/BrassRing = accountGateAdapter (detection
                + account-gate auto-create + email-verify wall; mostly standard HTML the generic engine
                already fills). Shared helpers: wdPickOption (Workday dropdowns), atsFillLocationTypeahead
                (location), accountGateAdapter + tickAccountAgreement + isVerifyEmailWall (account gates).
                ALL 8 target ATSs now have adapters; generic handles the rest.
